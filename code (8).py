...
def main():
    pygame.init()
    pygame.display.set_caption("3x3 Tarp Simulator - Drag and Stake")
    ...
    while True:
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                ...
                if event.button == 3:
                    ...
                    for i, idx in enumerate(tarp.get_tieout_indices()):
                        ...
                        if screen_pos and idx not in tarp.pinned_points: # Can't drag already pinned points
            ...
            if event.type == pygame.KEYDOWN:
                ...
                # --- CHANGE: New logic for staking and lifting ---
                elif event.key == pygame.K_s: # Stake
                    if dragged_tieout_info: # Priority: stake the dragged point
                        target_idx = dragged_tieout_info['index']
                        pos = np.copy(tarp.points[target_idx]); pos[1] = 0.0
                        tarp.pinned_points[target_idx] = pos
                        dragged_tieout_info = None # End the drag
                    else: # Fallback: stake the selected (yellow) point
                        target_idx = tarp.get_tieout_indices()[selected_tieout_index]
                        if target_idx not in tarp.pinned_points:
                            pos = np.copy(tarp.points[target_idx]); pos[1] = 0.0
                            tarp.pinned_points[target_idx] = pos
                            
                elif event.key == pygame.K_l: # Lift
                    if dragged_tieout_info: # Priority: lift the dragged point
                        target_idx = dragged_tieout_info['index']
                        pos = np.copy(tarp.points[target_idx]); pos[1] = POLE_HEIGHT
                        tarp.pinned_points[target_idx] = pos
                        dragged_tieout_info = None # End the drag
                    else: # Fallback: lift the selected (yellow) point
                        target_idx = tarp.get_tieout_indices()[selected_tieout_index]
                        if target_idx not in tarp.pinned_points:
                            pos = np.copy(tarp.points[target_idx]); pos[1] = POLE_HEIGHT
                            tarp.pinned_points[target_idx] = pos
        ...
        controls_text = [
            ...
            "Press S/L while dragging to lock a point!", "",
