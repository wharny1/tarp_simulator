...
class Tarp:
    def __init__(self, size, num_points):
        ...
        # Generate a list of spring constraints
        self.springs = []
    ...
    def reset(self):
        """ Resets the tarp to be PERFECTLY FLAT on the ground, with no wrinkles. """
        self.pinned_points.clear()
        step = self.size / (self.num_points - 1)
        for i in range(self.num_points):
            for j in range(self.num_points):
                # Tarp lies flat on the X-Z plane, slightly above y=0 to avoid visual glitches.
    ...
    def update(self, dt):
        ...
        for _ in range(SOLVER_ITERATIONS):
            ...
            # Ground constraint is crucial to keep it from falling forever.
...
def main():
    ...
    # NEW Camera state for Orthographic view
    zoom = 4.0
    pan_offset = [0.0, 0.0]
    ...
    while True:
        for event in pygame.event.get():
            ...
            # --- NEW Orthographic Controls ---
            if event.type == pygame.MOUSEBUTTONDOWN:
                if event.button == 1: mouse_dragging = True; last_mouse_pos = event.pos
                elif event.button == 4: zoom = max(1.0, zoom - 0.5) # Zoom in
                elif event.button == 5: zoom = min(10.0, zoom + 0.5) # Zoom out
            if event.type == pygame.MOUSEBUTTONUP:
                if event.button == 1: mouse_dragging = False
            if event.type == pygame.MOUSEMOTION and mouse_dragging:
                dx = event.pos[0] - last_mouse_pos[0]
                dy = event.pos[1] - last_mouse_pos[1]
                pan_offset[0] -= dx * 0.01 * (zoom/4.0) # Pan left/right
                pan_offset[1] += dy * 0.01 * (zoom/4.0) # Pan up/down
            ...
            if event.type == pygame.KEYDOWN:
                ...
                if event.key == pygame.K_r: tarp.reset(); pan_offset = [0,0]; zoom = 4.0
        ...
        # --- NEW Orthographic Camera Setup ---
        glMatrixMode(GL_PROJECTION)
        glLoadIdentity()
        aspect_ratio = WINDOW_WIDTH / WINDOW_HEIGHT
        glOrtho(-zoom * aspect_ratio, zoom * aspect_ratio, -zoom, zoom, -100, 100)

        glMatrixMode(GL_MODELVIEW)
        glLoadIdentity()
        # Look from above (0,10,0) at the center (pan_offset), with Z being "up" on the screen
        gluLookAt(pan_offset[0], 10, pan_offset[1],  # Eye position (high up)
                  pan_offset[0], 0,  pan_offset[1],  # Center of view
                  0, 0, -1)                         # Up vector
        ...
        controls_text = [
            "[3x3 Tarp Simulator - Top Down View]", "Left Click + Drag: Pan View", "Mouse Wheel: Zoom",
            "Left/Right Arrows: Select Tie-out", "S: Stake selected tie-out to ground",
            "L: Lift selected tie-out (pole)", "U: Un-pin selected tie-out", "R: Reset Tarp & View",
