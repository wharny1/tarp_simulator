...
class Tarp:
    ...
    def reset(self):
        """ Resets the tarp to a VERTICAL state, pinned by its top corners. """
        self.pinned_points.clear()
        step = self.size / (self.num_points - 1)
        for i in range(self.num_points):
            for j in range(self.num_points):
                # Create the tarp on the X-Y plane (vertically)
                self.points[i, j] = np.array([-self.size/2 + j*step, 3.0 - i*step, 0.0])
        self.old_points = np.copy(self.points)

        # CHANGE: Pin the top two corners so it hangs and is visible from the start.
    ...
    def draw(self, selected_tieout_idx):
        """ Draws the tarp mesh (without lighting), tie-outs, and guylines. """
        # --- Draw Tarp Surface (Solid Color) ---
        ...
        for i, tieout_idx in enumerate(all_tieouts):
            ...
            if tieout_idx in self.pinned_points:
                # Use a brighter color for pinned guylines to make the initial state clear
                glColor3f(1.0, 1.0, 1.0) # White for guylines
                glBegin(GL_LINES)
                glVertex3fv(pos)
                glVertex3fv(self.pinned_points[tieout_idx]) # Draw line to the pinned position
                glEnd()
...
def main():
    ...
    # Camera state
    camera_distance = 8.0 
    camera_rotation = [0, -15] # Front-on view
    ...
    while True:
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEMOTION and mouse_dragging:
                ...
                camera_rotation[1] = max(-89.9, min(89.9, camera_rotation[1])) # Clamp tilt
                last_mouse_pos = event.pos
            # --- Keyboard Controls ---
        ...
        # Center the pivot point on the hanging tarp (center is at y=1.5)
        glTranslatef(0.0, -1.5, -camera_distance)
