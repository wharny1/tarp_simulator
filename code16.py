...
def main():
    ...
    persp_dist, persp_rot, persp_pan = 6.0, [0, -89.9], [0.0, 0.0] 
    ...
    while True:
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEMOTION:
                ...
                # This block handles middle-click pan (both views) and left-click pan (ortho view)
                if mouse_dragging_pan or (mouse_dragging_camera and is_ortho_view):
                    if is_ortho_view:
                        # --- CHANGE: Inverted all panning directions for Ortho view ---
                        # Drag right -> pan left, drag down -> pan up
                        ortho_pan[0] -= dx * 0.01 * (ortho_zoom / 4.0)
                        ortho_pan[1] -= dy * 0.01 * (ortho_zoom / 4.0)
                    else: # Perspective pan (only for middle mouse)
                        scale = 0.005 * persp_dist
                        # --- CHANGE: Inverted horizontal (side-to-side) panning ---
                        persp_pan[0] -= dx * scale
                        persp_pan[1] -= dy * scale # Vertical pan remains standard (drag down -> pan down)
                    last_mouse_pos = event.pos

                # This block handles left-click orbit (perspective view)
                elif mouse_dragging_camera and not is_ortho_view:
                    persp_rot[0] += dx * 0.2 # Horizontal orbit remains standard
                    # --- CHANGE: Inverted vertical (up-down) orbit ---
                    # Drag down -> look down
                    persp_rot[1] -= dy * 0.2
        ...
        if is_ortho_view:
            ...
            # The signs were changed in the event handler, so gluLookAt remains the same
            ...
            glTranslatef(-persp_pan[0], -persp_pan[1], 0.0)
        ...
        if show_ui:
            ...
            # UI text does not need to change, as the actions are still named the same.
            controls_text = [
                f"View: {'Top-Down' if is_ortho_view else '3D Orbit'} (V)",
                "L-Click+Drag: Orbit (3D) / Pan (Top-Down)",
                "Scroll-Wheel+Drag: Pan View",
