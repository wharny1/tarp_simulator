...
POLE_HEIGHT = 1.2
# --- CHANGE: New constant to control how far the tarp can be stretched by dragging ---
MAX_STRETCH_FACTOR = 2.5 # Lower = stiffer, Higher = stretchier
...
class Tarp:
    ...
    # --- CHANGE: New helper method to find the center of a point's neighbors ---
    def get_neighbor_center(self, p_idx):
        total_pos = np.zeros(3)
        count = 0
        # This is a simple but clear way to find neighbors. For larger meshes,
        # a pre-computed neighbor list would be more efficient.
        for p1_idx, p2_idx, _ in self.springs:
            if p1_idx == p_idx:
                total_pos += self.points[p2_idx]
                count += 1
            elif p2_idx == p_idx:
                total_pos += self.points[p1_idx]
                count += 1
        
        if count > 0:
            return total_pos / count
        return self.points[p_idx] # Fallback if no neighbors found
    ...
    def update(self, dt, dragged_point_info=None):
        ...
        for _ in range(SOLVER_ITERATIONS):
            ...
            if dragged_point_info:
                ...
                # The target_pos is now constrained, but we still apply a force
                # to pull the point towards it for a 'springy' feel.
...
def main():
    ...
    persp_dist, persp_rot = 6.0, [0, 45] 
    persp_focus_point = np.array([0.0, 0.0, 0.0])

    selected_tieout_index = 4
    paused = False
    mouse_dragging_camera = False
    mouse_dragging_pan = False 
    last_mouse_pos = (0, 0)
    ...
    while True:
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                ...
                elif event.button == 2: mouse_dragging_pan = True; last_mouse_pos = event.pos
                elif event.button == 3:
                    mx, my = event.pos
            ...
            if event.type == pygame.MOUSEBUTTONUP:
                if event.button == 1: mouse_dragging_camera = False
                elif event.button == 2: mouse_dragging_pan = False

            if event.type == pygame.MOUSEMOTION:
                dx, dy = event.pos[0] - last_mouse_pos[0], event.pos[1] - last_mouse_pos[1]
                
                if mouse_dragging_pan or (mouse_dragging_camera and is_ortho_view):
                    if is_ortho_view:
                        ortho_pan[0] -= dx * 0.01 * (ortho_zoom / 4.0)
                        ortho_pan[1] -= dy * 0.01 * (ortho_zoom / 4.0)
                    else: 
                        scale = 0.002 * persp_dist
                        yaw_rad = np.radians(persp_rot[0])
                        right_vec = np.array([np.cos(yaw_rad), 0, -np.sin(yaw_rad)])
                        persp_focus_point -= right_vec * dx * scale
                        fwd_vec = np.array([-np.sin(yaw_rad), 0, -np.cos(yaw_rad)])
                        persp_focus_point += fwd_vec * dy * scale
                    last_mouse_pos = event.pos

                elif mouse_dragging_camera and not is_ortho_view:
                    persp_rot[0] -= dx * 0.2
                    persp_rot[1] += dy * 0.2 
                    persp_rot[1] = max(0.1, min(89.9, persp_rot[1]))
                    last_mouse_pos = event.pos

            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_v: is_ortho_view = not is_ortho_view
                elif event.key == pygame.K_e: show_ui = not show_ui
                    ...
                    target_idx = None
                    if dragged_tieout_info: target_idx = dragged_tieout_info['index']
                    else: target_idx = tarp.get_tieout_indices()[selected_tieout_index]
                    ...
                    if target_idx in tarp.pinned_points: del tarp.pinned_points[target_idx]
                
                elif event.key == pygame.K_a: selected_tieout_index = (selected_tieout_index - 1) % 9
                elif event.key == pygame.K_d: selected_tieout_index = (selected_tieout_index + 1) % 9
                elif event.key == pygame.K_w: selected_tieout_index = (selected_tieout_index - 3) % 9
                elif event.key == pygame.K_s: selected_tieout_index = (selected_tieout_index + 3) % 9
                
                elif event.key == pygame.K_SPACE: paused = not paused
        
        drag_info_for_update = None
        # --- CHANGE: Major logic update for constraining the drag position ---
        if dragged_tieout_info:
            drag_idx = dragged_tieout_info['index']
            mx, my = pygame.mouse.get_pos()
            mouse_target_pos = gluUnProject(mx, WINDOW_HEIGHT-my, dragged_tieout_info['depth'], modelview_matrix, projection_matrix, viewport)
            
            if mouse_target_pos:
                # Find the center of the point's neighbors to act as an anchor
                neighbor_center = tarp.get_neighbor_center(drag_idx)
                
                # Define the maximum distance from that anchor
                step = tarp.size / (tarp.num_points - 1)
                max_dist = step * MAX_STRETCH_FACTOR

                # Calculate vector from anchor to the mouse's world position
                vec_to_mouse = np.array(mouse_target_pos) - neighbor_center
                dist_to_mouse = np.linalg.norm(vec_to_mouse)
                
                final_target_pos = np.array(mouse_target_pos)
                # If the mouse is too far, clamp the target position
                if dist_to_mouse > max_dist:
                    # Normalize the vector and scale it to the max distance
                    clamped_vec = vec_to_mouse / dist_to_mouse * max_dist
                    final_target_pos = neighbor_center + clamped_vec
                
                # Use this (potentially clamped) position for the update
                drag_info_for_update = {'index': drag_idx, 'target_pos': final_target_pos}
        ...
        if is_ortho_view:
            ...
            yaw_rad, pitch_rad = np.radians(persp_rot[0]), np.radians(persp_rot[1])
            cam_x = persp_focus_point[0] + persp_dist * np.cos(pitch_rad) * np.sin(yaw_rad)
            cam_y = persp_focus_point[1] + persp_dist * np.sin(pitch_rad)
            cam_z = persp_focus_point[2] + persp_dist * np.cos(pitch_rad) * np.cos(yaw_rad)
            gluLookAt(cam_x, cam_y, cam_z,                 # Camera position
                      persp_focus_point[0], persp_focus_point[1], persp_focus_point[2], # Where to look
                      0, 1, 0)                             # Up vector
        ...
        if show_ui:
            ...
            controls_text = [
                f"View: {'Top-Down' if is_ortho_view else '3D Orbit'} (V)",
                "L-Click+Drag: Orbit (3D) / Pan (Top-Down)",
                "Scroll-Wheel+Drag: Pan View (Constrained in 3D)",
                "R-Click Tie-out: Toggle Drag (Now with stretch limit!)",
                "W/S/A/D: Select Tie-out (Yellow)",
                "Q: Pin in Place", 
                "U: Un-pin Selected",
                "R: Reset All",
                "SPACE: Pause", 
                "E: Toggle Instructions", "",
                f"Selected: {selected_tieout_index+1}/9",
                "Status: " + ("DRAGGING" if dragged_tieout_info else "Paused" if paused else "Running"),
                "Feedback: will@bartleys.net"]
