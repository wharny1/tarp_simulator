...
def main():
    ...
    persp_dist, persp_rot, persp_pan = 6.0, [0, -89.9], [0.0, 0.0] # --- CHANGE: Added persp_pan

    selected_tieout_index = 4
    paused = False
    mouse_dragging_camera = False
    mouse_dragging_pan = False # --- CHANGE: New state for middle-mouse drag
    last_mouse_pos = (0, 0)
    ...
    while True:
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                ...
                # --- CHANGE: Added handler for middle mouse button (scroll wheel) ---
            ...
            if event.type == pygame.MOUSEBUTTONUP:
                ...
                # --- CHANGE: Handle middle mouse button release ---
                elif event.button == 2: mouse_dragging_pan = False

            # --- CHANGE: Reworked MOUSEMOTION to handle all drag types ---
            if event.type == pygame.MOUSEMOTION:
                dx, dy = event.pos[0] - last_mouse_pos[0], event.pos[1] - last_mouse_pos[1]

                # Pan camera if middle-dragging, or if left-dragging in ortho view
                if mouse_dragging_pan or (mouse_dragging_camera and is_ortho_view):
                    if is_ortho_view:
                        # Drag right -> pan right, drag down -> pan down
                        ...
                        persp_pan[1] -= dy * scale # Screen Y is inverted from OpenGL Y
                    last_mouse_pos = event.pos

                # Orbit camera if left-dragging in perspective view
            ...
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_v: is_ortho_view = not is_ortho_view
                    ...
                    persp_dist, persp_rot, persp_pan = 6.0, [0, -89.9], [0.0, 0.0] # --- CHANGE: Reset pan
                    dragged_tieout_info = None
        ...
        if is_ortho_view:
            ...
            # --- CHANGE: Apply perspective pan transformation ---
        ...
        if show_ui:
            ...
            # --- CHANGE: Updated UI text for new controls ---
            controls_text = [
