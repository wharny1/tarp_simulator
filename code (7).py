...
TARP_SIZE = 3.0
POINTS_PER_AXIS = 17
SOLVER_ITERATIONS = 10 # Increased for better drag stability
...
class Tarp:
    ...
    # --- CHANGE: Update now accepts information about the dragged point ---
    def update(self, dt, dragged_point_info=None):
        # 1. Verlet integration (unchanged)
        for i in range(self.num_points):
            for j in range(self.num_points):
        ...
        # 2. Constraint satisfaction loop
        for _ in range(SOLVER_ITERATIONS):
            # Tarp's internal springs (unchanged)
            ...
            # --- NEW: "Soft drag" constraint ---
            # This acts like a spring pulling the point towards the mouse
            if dragged_point_info:
                drag_idx = dragged_point_info['index']
                target_pos = dragged_point_info['target_pos']
                # Vector from the point to the mouse target
                correction_vec = target_pos - self.points[drag_idx]
                # Move the point towards the target by a fraction (drag strength)
                self.points[drag_idx] += correction_vec * 0.8
            
            # Pinned point constraints (unchanged)
            for idx, pos in self.pinned_points.items():
                self.points[idx] = pos
                
            # Ground constraint (unchanged)
...
def main():
    pygame.init()
    pygame.display.set_caption("3x3 Tarp Simulator - Physically-based Dragging")
    ...
    while True:
        ...
        # --- Create drag info for the physics update ---
        ...
        # --- Call the updated physics function ---
        if not paused:
        ...
        controls_text = [
            ...
            "Drag force is now physically simulated.",
            "S: Stake", "L: Lift", "U: Un-pin", "R: Reset", "SPACE: Pause", "",
