...
def draw_ground():
    ...
    for i in np.arange(-10, 10.1, 0.5): # Increased ground size for panning
...
def main():
    ...
    # --- CHANGE: Reworked perspective controls for gluLookAt ---
    persp_dist, persp_rot = 6.0, [0, 45] # Note: y-rot is now pitch, start at 45 deg
    persp_focus_point = np.array([0.0, 0.0, 0.0]) # The point on the ground we look at
    ...
    while True:
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEMOTION:
                ...
                # Orbit camera (perspective L-click only)
                elif mouse_dragging_camera and not is_ortho_view:
                    persp_rot[0] += dx * 0.2
                    # --- CHANGE: Reverted to original non-inverted vertical orbit ---
                    persp_rot[1] += dy * 0.2 
                    persp_rot[1] = max(0.1, min(89.9, persp_rot[1])) # Clamp pitch
                    last_mouse_pos = event.pos
            ...
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_v: is_ortho_view = not is_ortho_view
                    ...
                    persp_dist, persp_rot = 6.0, [45, 45]
        ...
        if is_ortho_view:
            ...
            glMatrixMode(GL_MODELVIEW); glLoadIdentity()
            # --- CHANGE: Replaced matrix stack with gluLookAt for easier control ---
            yaw_rad = np.radians(persp_rot[0])
            pitch_rad = np.radians(persp_rot[1])

            # Calculate camera position based on distance, rotation, and focus point
