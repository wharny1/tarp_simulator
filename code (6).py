...
class Tarp:
    ...
    def draw(self, selected_tieout_idx, dragged_tieout_idx=None):
        ...
        for i, tieout_idx in enumerate(all_tieouts):
            ...
            # NEW: Special color for the dragged point
            if tieout_idx == dragged_tieout_idx:
                glColor3f(0.0, 1.0, 1.0); glPointSize(20) # Cyan for dragged
            elif i == selected_tieout_idx: 
                glColor3f(1.0, 1.0, 0.0); glPointSize(15) # Yellow for selected
            else: 
                glColor3f(1.0, 0.5, 0.0); glPointSize(10) # Orange for others
...
def main():
    pygame.init()
    pygame.display.set_caption("3x3 Tarp Simulator - Right-click to drag tie-outs")
    ...
    # --- NEW: State for dragging a point ---
    dragged_tieout_info = None # Will store {'index': (i,j), 'depth': screen_z}

    while True:
        # Get matrices and viewport once at the start of the frame for picking/unprojecting
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                ...
                # --- NEW: Right-click logic for picking/dropping tie-outs ---
                if event.button == 3: # Right mouse button
                    mx, my = event.pos
                    # Find the closest tie-out to the mouse click
                    closest_tieout = None
                    min_dist_sq = 15**2 # Click within 15 pixels
                    ...
                    for i, idx in enumerate(tarp.get_tieout_indices()):
                        ...
                        if screen_pos:
                            ...
                            if dist_sq < min_dist_sq:
                                min_dist_sq = dist_sq
                                closest_tieout = {'index': idx, 'depth': screen_pos[2]}
                    
                    if closest_tieout:
                        # If we clicked the currently dragged point, drop it.
                        if dragged_tieout_info and dragged_tieout_info['index'] == closest_tieout['index']:
                            dragged_tieout_info = None
                        else: # Otherwise, pick up the new point.
                            dragged_tieout_info = closest_tieout
            ...
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_v: is_ortho_view = not is_ortho_view
                    ...
                    persp_dist, persp_rot = 6.0, [0, -89.9]
                    dragged_tieout_info = None # Drop any dragged point on reset
                # ... (other keys) ...
                ...
                if event.key == pygame.K_s and not dragged_tieout_info:
                ...
                if event.key == pygame.K_l and not dragged_tieout_info:
                     if current_tieout not in tarp.pinned_points:
        ...
        # --- NEW: If a point is being dragged, update its position ---
        if dragged_tieout_info:
            mx, my = pygame.mouse.get_pos()
            # Unproject the 2D mouse position back into 3D world space
            world_pos = gluUnProject(mx, WINDOW_HEIGHT - my, dragged_tieout_info['depth'], modelview_matrix, projection_matrix, viewport)
            if world_pos:
                # Force the dragged point to the new 3D position
                tarp.points[dragged_tieout_info['index']] = np.array(world_pos)
        ...
        controls_text = [
            ...
            "L-Click+Drag: " + ("Pan" if is_ortho_view else "Orbit"),
            "R-Click Tie-out: Toggle Drag",
            "Mouse Wheel: Zoom", "S: Stake", "L: Lift", "U: Un-pin",
